...
public class Client extends PircBotX {
	...
	@Override
	public synchronized void connect(String hostname, int port, String password, SocketFactory socketFactory) throws IOException, IrcException,
		...
		if (this.webIrcPassword != null) {
			this.outputThread.sendRawLineNow("WEBIRC " + this.webIrcPassword + " cgiirc " + this.webIrcHostname + " "
					+ this.webIrcAddress.getHostAddress());
		}
		...
		while ((line = breader.readLine()) != null) {
			...
			if (secondSpace >= 0) {
				...
				// Check for both a successful connection. Inital connection
				// (001-4), user stats (251-5), or MOTD (375-6)
				...
				if (Arrays.asList(codes).contains(code)) {
					...
					// Iterate through all available nicknames. If we reach the
					// end,
					...
					// Example: Client: Target change too fast. Please wait 104
					// seconds.
					// No action required.
		...
		// This makes the socket timeout on read operations the set timeout,
